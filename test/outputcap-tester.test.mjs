...
test('CLI show: grok reports its raised built-in (2 000 000); gemini/codex report the fleet default (400 000)', () => {
  const dir = home();
  const grok = cli(['show', 'grok'], { dir });
  assert.equal(grok.code, 0);
  assert.match(grok.out, /^\s+outputCap\s+2000000\s+default$/m);
...
test('CLI set: grok.outputCap=1000 is accepted, persisted, and read back by show', () => {
  ...
  assert.match(s.out, /grok\.outputCap\s+2000000 \[default\] → 1000 \[file\]/);
...
test('CLI set: grok.outputCap=0 and grok.outputCap=abc are both refused — exit 1, nothing written', () => {
  ...
  const shown = cli(['show', 'grok'], { dir });
  assert.match(shown.out, /^\s+outputCap\s+2000000\s+default$/m);
});
