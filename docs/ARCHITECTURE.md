...
| `rules.mjs` | Every text that tells a session how to work with the fleet: `FLEET_CONTRACT` and `unitInstructions()` behind each server's `initialize.instructions`, plus the `rules/` and `agents/` templates, their version markers and their target paths — the marker is what makes a file the CLI's to refresh or remove. |
