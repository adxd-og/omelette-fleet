...
| `webSearch` | boolean | `true` | Whether the unit's web tools are available. grok: affects research only (review is local-only and never has web tools); `false` makes `grok_research` refuse (it is web-only and never gets read tools instead). codex: affects research only; review is always without |
...
| `webSearch` | — | research: `web_search,web_fetch`, or refused when `false`; review: never web | research: the CLI's own web mode, or `-c web_search="disabled"` when `false`; review: always `"disabled"` |
