...
Doctor resolves the variable the way the client does — the process environment first, then the `env` block of `.claude/settings.local.json`, `.claude/settings.json`, and the user scope's files under `$CLAUDE_CONFIG_DIR` or `~/.claude` — and names the file it came from, because a wall read out of the wrong file is worse than no reading at all. A file that exists and does not parse is named rather than skipped in silence — `settings: <path> unreadable — env values in it were not consulted`, once per file however many of doctor's readers opened it — because a variable written into a file with a trailing comma is a variable the client never saw.
