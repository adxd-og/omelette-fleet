...
- [ ] **Fail loudly, never blankly.** An empty answer with a talkative stderr is a bug report, not a shrug — surface the stderr. A run that produced text but exited non-zero, stopped early, or was hard-killed keeps the text and appends a marker (`[<unit>: CLI exited N — treat the answer as partial]`, `[<unit>: hard-killed after <N>s — … raise <unit>.timeoutS …]`); only a failure the parser cannot read an answer out of throws — no text, a vendor-reported failed turn (Codex `turn.failed`), a quota refusal. Build every marker with `partialMark` and append it with `withPartial` from `core/partial.mjs`: the marker and `partial: true` are one operation, and the flag reaches the status feed's `end` event and the spool header — beside `status: "ok"`, or `"cancelled"` when the client is what ended the run. Extract that text with the SAME parsing the success path uses — a second parser for the failure path is how the two drift. Refusals you handle yourself return `isError: true`.
