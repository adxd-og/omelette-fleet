...
It has no dependencies, makes no network calls, and **never fails a session**: malformed stdin, stdin that never arrives, an unrecognised event, a ledger it may not write — all of them exit 0. Stdin is bounded in both directions, because it is somebody else's pipe: more than 1 MiB is read as malformed and stops the read, and a pipe that never closes is abandoned after 5 seconds. Both exit 0. A guard that crashes takes the session with it, and this one is protecting exactly one thing.
