...
- **`PreToolUse`, matcher `Bash`.** It reads `agent_type` from the event on stdin. When the caller is `omelette-coder` *and* the command is a `git` invocation of `commit`, `push`, `stash`, `worktree`, `checkout -b` or `switch -c` — with any leading `-C <dir>`, `-c key=value` or `--flag` options in between, so prefixing them does not slip past it — it writes one line to stderr and exits **2**, which is how a hook refuses a tool call and hands the reason back to that agent. Reads are deliberately untouched (`git log`, `git diff`, `git status`), and so is `git commit-tree`, which writes an object and commits nothing. Every other caller, the main session included, and every other command: exit 0, nothing said.
- **`PreCompact`.** It appends `## Compaction <ISO timestamp> (trigger: …) — re-read this ledger before continuing` to every file matching `.omelette/ledger-*.md` under the event's `cwd`, and prints a one-line handoff reminder on stdout. Nothing else in the project is read or written.

It has no dependencies, makes no network calls, and **never fails a session**: malformed stdin, stdin that never arrives, an unrecognised event, a ledger it may not write — all of them exit 0. A guard that crashes takes the session with it, and this one is protecting exactly one thing.
