...
- **`check` never leaves the project root and never writes.** `omelette-fleet check` reads the markdown file it is given and the files its pointer lines name — resolved against the real root, refused when they leave it, symlinks and non-regular files refused, 2 MiB per target — and runs one `git diff --name-only <hash>..HEAD` (no shell; the hash is validated as hex first). It writes nothing.
