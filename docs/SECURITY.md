...
- **`PreToolUse`, matcher `Bash`.** It reads `agent_type` from the event on stdin. When the caller is `omelette-coder` *and* the command is a `git` invocation that creates commits, moves branches or touches stashes, worktrees or tags — `commit`, `merge`, `rebase`, `cherry-pick`, `revert`, `am`, `pull`, `push`, `stash`, `tag`, `worktree`, `branch <name>` or a `branch` flag that moves a ref (`-m`/`-M`, `-c`/`-C`, `-f`/`--force`, `-u`/`--set-upstream-to`), or a `checkout`/`switch` that creates one (`-b`/`-B`, `-c`/`-C`, `--create`, `--force-create`, `--orphan`, `--track`, wherever in the command the flag sits and whether or not the name is attached to it) — it writes one line to stderr and exits **2**, which is how a hook refuses a tool call and hands the reason back to that agent. Any leading options are stepped over on the way to the subcommand, quoted values included (`-C "/My Dir"`, `-c user.name="A B"`, `--work-tree /tmp/x`, a value-less `-p`, a bare `--`), so prefixing them does not slip past it. Reads are deliberately untouched (`git log`, `git diff`, `git status`, `git branch` listing, deleting or with `-a`/`-r`/`-v`), and so is `git commit-tree`, which writes an object and commits nothing. Every other caller, the main session included, and every other command: exit 0, nothing said.
...
**A hook script is inert until your settings call it, and this package does not write those files.** `rules --hooks` prints the snippet — the absolute script path, shell-quoted, because a hook `command` is a command line and not an argv — and you paste it into `.claude/settings.json` or `.claude/settings.local.json` yourself. `doctor` parses **both** of a scope's settings files read-only to report whether the guard is wired, and matches on the script's *name* inside the command string rather than on an exact path, so quoting it (the printed snippet does), wrapping it or keeping it elsewhere still reads as wired. A settings file that exists but does not parse is reported as unreadable by name rather than silently counted as "not wired". Settings-level is also the only placement that works: a `hooks:` block in an agent definition's own frontmatter did not fire at all on Claude Code 2.1.261, while the same hook in the project's settings fired for the same sub-agent and carried `agent_type` on its stdin (measured 2026-09-06).
