...
| `lastEvent.partial` | `true` only when the answer is incomplete, for one of five reasons: the run outlived `timeoutS` and was SIGKILLed with text already produced; its stdout hit `outputCap` and the beginning of the output was dropped; the **client cancelled** the call and the unit's `cancel` is `kill`, with text already produced; a `gemini_deep_research` report was synthesized from stages of which at least one was itself partial; or an **image run** did not finish cleanly — capped, killed, cancelled, a non-zero exit, a vendor status that is not success — and had already saved its file. `status` is `"ok"` where there is an answer to read, and `"cancelled"` where the client is what ended the run. The text carries the matching marker — `[<unit>: hard-killed after <N>s …]`, `[<unit>: output capped at <N> chars …]`, `[<unit>: cancelled by the client …]`, `[gemini: N of M stages returned partial answers]` — with more than one of them on a run that managed more than one. **The image tools carry none**: their answer is a bare path by contract (`gemini_image`, `grok_image`, `grok_image_edit`, `codex_image`), so this flag is the only record that the run behind that file was cut short. Absent on a normal call — never `false` |
