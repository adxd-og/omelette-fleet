...
| `check <file.md> [--strict] [--require <n>] [--root <dir>]` | Verifies the pointer lines of a sub-agent report or a scout map — `path:line` · a verbatim fragment in backticks · the claim: each is `ok`, `moved` (with the line it was found at), `mismatch`, `missing`, `outside` the root, `too-large`, or `stale` when the file changed since the map's `commit:`. Exit `0` only when every pointer holds and there is at least one (`--require 0` for a report that may have none; `--strict` also fails on `stale`). Read-only. See [Evidence with pointers](docs/ORCHESTRATION.md#evidence-with-pointers) |
