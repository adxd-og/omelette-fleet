...
| `doctor [--prefix <name>] [--probe-models]` | Per unit: binary, `--version`, login state, resolved config with sources, ceiling, MCP registration, status-feed writability. Above them: the managed files at both scopes, and — while anything is missing — one `next` line (`install` → `rules --agents --hooks` → merge the snippet), which is a hint and never a fault. A registration counts as yours only if its command is node and its path is *this* clone's `servers/<unit>.mjs` — otherwise it is reported as "registered elsewhere". `--probe-models` spends real Codex calls to test every catalog id |
