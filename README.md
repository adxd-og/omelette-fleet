...
Optionally, install the guard hook — the coder's "never commits" rule, enforced rather than requested, plus a ledger marker on every compaction and the last handoff block printed back into the session that follows one:
